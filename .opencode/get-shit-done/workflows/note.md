...
- **Global scope**: `E:/Curso OpenCode/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `E:/Curso OpenCode/.opencode/notes/`)
...
2. Glob `E:/Curso OpenCode/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (E:/Curso OpenCode/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `E:/Curso OpenCode/.opencode/notes/` — works in any directory
